...
export default function PurchaseDocumentsPage() {
  ...
  const create = api.sales.createPurchaseDocument.useMutation({
    onSuccess: () => utils.sales.invalidate(),
  });
  const list = api.sales.purchaseDocuments.useQuery({
    docType: 'PURCHASE',
    page,
    pageSize,
  ...
  const columns: Column<Row>[] = [
    ...
    { key: 'partner', header: '매입처', render: (r) => r.partner.name },
    { key: 'warehouse', header: '입고창고', width: 120, render: (r) => r.warehouse.name },
  ...
  async function submit() {
    ...
    if (!header.partnerId)
      problems.push({ field: 'pd-partner', label: '매입처', message: '매입처를 선택하세요.' });
    if (!header.warehouseId)
      problems.push({ field: 'pd-wh', label: '입고창고', message: '입고창고를 선택하세요.' });
    if (entered.length === 0) {
      problems.push({ field: 'pd-lines', label: '품목', message: '품목을 한 건 이상 입력하세요.' });
    } else if (resolved && 'errors' in resolved) {
      for (const message of resolved.errors) problems.push({ field: 'pd-lines', label: '품목', message });
    }
    if (problems.length) {
      setErrors(problems);
      return;
  ...
  return (
    <StandardListPage
      title="매입전표"
      description="확정 시 입력한 매입단가로 재고가 들어오고, 미지급금과 회계 분개가 함께 생성됩니다."
      filterKey="sales.purchaseDocuments"
      actions={
      ...
      filters={
        <>
          <Field label="검색어" htmlFor="pdf-q" hint="전표번호·매입처명">
            <Input
              id="pdf-q"
              value={filters.q}
          ...
          <Field label="매입처" htmlFor="pdf-partner">
            <PartnerSelect
              id="pdf-partner"
              includeAll
          ...
          <Field label="상태" htmlFor="pdf-status">
            <Select
              id="pdf-status"
    ...
    >
      {creating ? (
        <Card title="매입전표 등록" className="mb-4">
          <FormErrorSummary errors={errors} />
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
            <Field label="전표일" htmlFor="pd-date" required>
              <Input
                id="pd-date"
                type="date"
            ...
            <Field label="매입처" htmlFor="pd-partner" required>
              <PartnerSelect
                id="pd-partner"
                partnerType="SUPPLIER"
            ...
            <Field label="입고창고" htmlFor="pd-wh" required>
              <Select
                id="pd-wh"
            ...
            <Field label="비고" htmlFor="pd-note">
              <Input
                id="pd-note"
                value={header.note}
          ...
          <div className="mt-4" id="pd-lines">
